				               ...
				               int *pitches, int *offsets)
{
    ...
    xf86DrvMsgVerb(0, X_INFO, DefaultLevel, 
                   "==RDCQueryImageAttributesOverlay()  Entry; Width = %d, Height = %d==\n", 
                   *w, *h);
...
RDCStopVideoPost(ScrnInfoPtr pScrn, pointer data, Bool shutdown)
{
    ...
    if (shutdown)
    {
        ...
        else
        {
            ...
            if (pRDC->IoctlCR)
            {
                pSingleCMD = (PKT_SC *) malloc(PKT_SINGLE_LENGTH*10);
                pCMDQ = pSingleCMD;
            }
            else
    ...
    case FOURCC_YV12:
        {
            if(bRDC_Video)
            {
                ...
                unsigned char *pSubImage;
                unsigned char *pImage;
               
                pY = pRDC->FBVirtualAddr + pVideoInfo->luma;
                pCb = pRDC->FBVirtualAddr + pVideoInfo->chroma;
                pCr = pRDC->FBVirtualAddr + pVideoInfo->v;
                ...
                org_w = pVideoInfo->org_w;
                org_h = pVideoInfo->org_h;
                ...
                if (width !=0 && height !=0)
                {
                    ...
                    for (i=xpos ; i<xpos+width ; i++ )
                    {
                        ...
                        {
                            pImage = pSubImage + ((j-ypos) * pVideoInfo->w + (i-xpos));
                ...
                pVPHwInfo->ulVP_SrcBuf[0] = pRDCPortPriv->PackedBufOffset[0] = pVideoInfo->luma;
                pVPHwInfo->ulVP_SrcBuf[1] = pRDCPortPriv->PackedBufOffset[1] = pVideoInfo->chroma;
                pVPHwInfo->ulVP_SrcBuf[2] = pRDCPortPriv->PackedBufOffset[2] = pVideoInfo->v;
            }
            else
            {
                ULONG   ulTempH;
...
void UpdateVPost(RDCRecPtr pRDC, VPOSTHWINFOPtr pVPHwInfo)
{
    ...
    else
    {
        ...
        if (pRDC->IoctlCR)
        {
            pSingleCMD = (PKT_SC *) malloc(PKT_SINGLE_LENGTH*14);
            pCMDQ = pSingleCMD;
        }
        else
        ...
        if (pRDC->IoctlCR)
            FireCRCMDQ(pRDC->iFBDev, pCMDQ, 14);
        else
            mUpdateWritePointer;
    }
}
...
#if CR_FENCE_ID
            WORD wFence;
            pRDC->CMDQInfo.bFuncType = DDBLT;
            wFence = ((WORD)pRDC->CMDQInfo.bFuncType) << 8 | 
                      (WORD)(pRDC->CMDQInfo.bFenceID++);
            
            pSingleCMD = (PKT_SC *) pjRequestCMDQ(pRDC, PKT_SINGLE_LENGTH*13);
#else
            PKT_SC *pCMDQ;
            if (pRDC->IoctlCR)
            {
                pSingleCMD = (PKT_SC *) malloc(PKT_SINGLE_LENGTH*12);
                pCMDQ = pSingleCMD;
            }
            else
...
#if CR_FENCE_ID
            WORD wFence;
            pRDC->CMDQInfo.bFuncType = SOURCE_IN_QUEUE;
            wFence = ((WORD)pRDC->CMDQInfo.bFuncType) << 8 | 
                      (WORD)(pRDC->CMDQInfo.bFenceID++);
...
void RDCUpdateVideo(RDCRecPtr pRDC, RDCPortPrivPtr pRDCPortPriv, BYTE bEnable, Bool bRDC_Video)
{
    ...
    if(bRDC_Video)
    {
        dwVPStreamCtrl = VPS_STREAM_NV12;        
        pRDC->dwMaxVPOSTWidth = VP_SRC_MAX_WIDTH;
    }
    else
    {
        ...
        switch (pRDCPortPriv->YUVFormat)
        {
            ...
            case FOURCC_I420:
                ...
                pRDC->dwMaxVPOSTWidth = PATCH_YV12_WIDTH;
                break;
        }
    }
    ...
    if (!pRDC->OverlayStatus.bOverlayEnable)
    {
        ...
        else
        {
            ...
            pSingleCMD->PKT_SC_dwHeader  = (ULONG)(CR_LOCK_ID | CR_LOCK_2DDMA_ID); 
            pSingleCMD->PKT_SC_dwData[0] = (ULONG)(CR_LOCK_2D);
            pSingleCMD++;
            pSingleCMD->PKT_SC_dwHeader  = (ULONG) PKT_VIDEO_CMD_HEADER | VDP_CTL_I;
    ...
    {
        if (ulVPOSTVideoSrcHorSize <= pRDC->dwMaxVPOSTWidth)
        {
    ...
    if (ulOverlayDstHorSize > ulOverlaySrcHorSize)
    {
        ...
        if (ulOverlaySrcHorSize > pRDC->dwMaxVPOSTWidth)
    ...
    do
    {
        if (ulVPOSTVideoSrcHorSize > pRDC->dwMaxVPOSTWidth)
        {
            pVPHwInfo->ulVP_VideoSize = ((pRDC->dwMaxVPOSTWidth - 1) << 16) | (ulVPOSTVideoSrcVerSize - 1);
            ...
            if (pRDC->bRandRRotation)
            {
                switch (pRDC->rotate)
                {
                    case RR_Rotate_0:
                        ...
                        break;
                    case RR_Rotate_90:
                        pVPHwInfo->ulVP_DestBuf_Offset[0] = ulVP_DestBuf_Offset_0 + dwVSrcPitch * (ulVPOSTVideoSrcHorSize - pRDC->dwMaxVPOSTWidth);
                        pVPHwInfo->ulVP_RotateCtrl = BIT31 | ((pRDCPortPriv->ulVPOSTVideoDstHorSize -1) << 16) | (pRDC->dwMaxVPOSTWidth -1);
                        break;
                    case RR_Rotate_180:
                        pVPHwInfo->ulVP_DestBuf_Offset[0] = ulVP_DestBuf_Offset_0 + ((ulVPOSTVideoSrcHorSize - pRDC->dwMaxVPOSTWidth) << 1);
                        pVPHwInfo->ulVP_RotateCtrl = BIT31 | ((pRDC->dwMaxVPOSTWidth -1) << 16) | (pRDCPortPriv->ulVPOSTVideoDstVerSize -1);
                        break;
                    case RR_Rotate_270:
                        pVPHwInfo->ulVP_RotateCtrl = BIT31 | ((pRDCPortPriv->ulVPOSTVideoDstHorSize -1) << 16) | (pRDC->dwMaxVPOSTWidth -1);
                        break;
        ...
        else
        {
            ...
            if (pRDC->bRandRRotation)
            {
                if (pRDCPortPriv->ulVPOSTVideoSrcHorSize > pRDC->dwMaxVPOSTWidth)
                {
                    switch (pRDC->rotate)
                    {
                        case RR_Rotate_0:
        ...
        ulVPOSTVideoSrcHorSize -= pRDC->dwMaxVPOSTWidth;
        ...
        if (pRDC->bRandRRotation)
        {
            ...
            {
                case RR_Rotate_0:
                    pVPHwInfo->ulVP_DestBuf_Offset[0] += (pRDC->dwMaxVPOSTWidth << 1);
                    break;
                case RR_Rotate_270:
                    pVPHwInfo->ulVP_DestBuf_Offset[0] += (dwVSrcPitch * pRDC->dwMaxVPOSTWidth);
                    break;
            }
        }
        else
        ...
        pVPHwInfo->ulVP_SrcBuf[0] += pRDC->dwMaxVPOSTWidth;
        pVPHwInfo->ulVP_SrcBuf[1] += (pRDC->dwMaxVPOSTWidth / 2);
        pVPHwInfo->ulVP_SrcBuf[2] += (pRDC->dwMaxVPOSTWidth / 2);
    }
...
void RDCAllocateVPOSTMem(ScrnInfoPtr pScrn, RDCPortPrivPtr pRDCPortPriv, long width, long height, Bool bRDC_Video)
{
    ...
    if(bRDC_Video)
    {
        ...
        pRDCPortPriv->PackedBufStride[0] = ALIGN_TO_8(width);
        pRDCPortPriv->PackedBufStride[1] = ALIGN_TO_8(width);
    }
	     ...
	     DrawablePtr pDraw)
{
    RDCRecPtr       pRDC = RDCPTR(pScrn);
    RDCPortPrivPtr  pRDCPortPriv = (RDCPortPrivPtr)data;
    VideoInfo *pVideoInfo = (VideoInfo *) buf;
    ...
    OV_RECTL        rDst, rSrc;
    Bool            bRDC_Video = FALSE;
    Bool            bEnableDual = FALSE;
    
    xf86DrvMsgVerb(0, X_INFO, DefaultLevel, "==RDCPutImageVPOST()  Entry==\n");

    if(pRDC->SupportDualPath)
    {
        bEnableDual = GetDisaplyStatus(pScrn);
   
        if(bEnableDual)
        {
            xf86CrtcConfigPtr xf86_config = XF86_CRTC_CONFIG_PTR(pScrn);        
            xf86CrtcPtr       crtc2 = xf86_config->crtc[1];
            short             HDisplay2_range = (short)crtc2->mode.HDisplay - (drw_w/2);
            if(drw_x > HDisplay2_range)
            {
                drw_x = drw_x - (short)crtc2->mode.HDisplay;
                ldrw_x = (long)drw_x;
                pRDC->SetVideoDisplay = VDPS_DISP2;
            }
            else
                pRDC->SetVideoDisplay = VDPS_DISP1;
        }
        else
            pRDC->SetVideoDisplay = VDPS_DISP1;
    ...
    else
    {
        ...
        {            
            ...
            xf86CrtcPtr       crtc2 = xf86_config->crtc[1];
            if(pRDC->SetVideoDisplay == VDPS_DISP2)
            {
                ModeWidth = crtc2->mode.HDisplay;
                ModeHeight = crtc2->mode.VDisplay;
            }
            else
            {                
                ModeWidth = crtc1->mode.HDisplay;
                ModeHeight = crtc1->mode.VDisplay;
            }
        }
        else
        {
    ...
    if(pVideoInfo->Maj_ID == 0x55AA55AA && pVideoInfo->Sec_ID == 0xFF00FF00 )
       bRDC_Video = TRUE;

    
    RDCAllocateVPOSTMem(pScrn, pRDCPortPriv, lwidth, lheight, bRDC_Video);
    
    if(bRDC_Video)
    {
        
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->luma : %8X\n", pVideoInfo->luma);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->chroma : %8X\n", pVideoInfo->chroma);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->v : %8X\n", pVideoInfo->v);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->size : %8X\n", pVideoInfo->size);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->x : %d\n", pVideoInfo->x);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->y : %d\n", pVideoInfo->y);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->w : %d\n", pVideoInfo->w);
        xf86DrvMsgVerb(0, X_ERROR, 0, "pVideoInfo->h : %d\n", pVideoInfo->h);
        RDCCopyFourCCVPOST(pRDC, pRDCPortPriv, (unsigned char *)pVideoInfo, lwidth, lheight, bRDC_Video);
    }
    else
    {
        RDCCopyFourCCVPOST(pRDC, pRDCPortPriv, buf, lwidth, lheight, bRDC_Video);
    }
    
    RDCUpdateVideo(pRDC, pRDCPortPriv, 1, bRDC_Video);

    xf86DrvMsgVerb(0, X_INFO, DefaultLevel, "==RDCPutImageVPOST()  Exit==\n");
